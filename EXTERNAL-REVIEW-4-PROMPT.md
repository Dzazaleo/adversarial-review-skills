...
| | |
|---|---|
| **Read** | Everything in the repository, including `.git` history and `~/.claude/skills/` if you want to compare the installed copies. Read outside the in-scope set freely for context |
...
**In one sentence: read anything, run the tests and anything under `/tmp`, write your report to
`EXTERNAL-REVIEW-4.md`, and modify nothing else.**
